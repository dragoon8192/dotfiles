...
#text
typeset c_tri_r='%1{%}' c_tri_l='%1{%}' c_git='%1{%}' c_cd='%2{❖%}'
# typeset c_tri_r='' c_tri_l='' c_git='' c_cd='❖ '
# Associative array
typeset -i size_aa=8
typeset -A aa_nor=([fc]=15 [kc]=9 [str]='%4{標準%}' [dir]='r')
typeset -A aa_ins=([fc]=15 [kc]=6 [str]='%4{入力%}' [dir]='r')
if [[ -n "${REMOTEHOST}${SSH_CONNECTION}" ]];then
    typeset -A aa_ssh=([fc]=15 [kc]=3 [str]='%4{遠隔%}' [dir]='r')
else
    typeset -A aa_ssh=()
fi
if [[ $UID == 0 ]];then
    typeset -A aa_root=([fc]=15 [kc]=10 [str]='%4{管理%}' [dir]='r')
else
    typeset -A aa_root=()
fi
